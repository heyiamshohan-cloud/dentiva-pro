...
function listQuery(collection, overrides = {}) {
  ...
  const { sort, ...filterOverrides } = overrides;
  return q('list', { collection, page: state.page, pageSize: state.pageSize, query: state.query || '', sort: sort ?? state.sort ?? '', filters: { ...state.filters, ...filterOverrides } });
}
...
/* ------------------------------- patients ------------------------------ */
const PATIENT_SORTS = [['name', 'Name A–Z'], ['name-desc', 'Name Z–A'], ['recent', 'Most recent'], ['oldest', 'Longest registered'], ['balance', 'Highest due'], ['billed-desc', 'Highest lifetime bill'], ['billed', 'Lowest lifetime bill'], ['paid-desc', 'Highest lifetime paid'], ['visits-desc', 'Most visits'], ['code', 'Patient code']];
const PATIENT_COLUMN_DEFS = [
  ['patient', 'Patient', true], ['phone', 'Phone', true], ['age', 'Age/Sex', false], ['lastVisit', 'Last visit', true],
  ['nextVisit', 'Next appointment', false], ['visits', 'Visits', false], ['billed', 'Lifetime bill', false],
  ['paid', 'Paid', false], ['balance', 'Due', true], ['lastPayment', 'Last payment', false], ['status', 'Status', true]
];
function patientColumnState() {
  const state = {};
  PATIENT_COLUMN_DEFS.filter(([k]) => k !== 'patient' && k !== 'status').forEach(([k, , d]) => { state[k] = d; });
  return state;
}
function persistUiPreference() {
  try { localStorage.setItem('dentiva:ui-prefs', JSON.stringify({ patientColumns: ui.patientColumns || null, patientDensity: ui.patientDensity || 'comfortable' })); } catch { /* storage unavailable — prefs live for session only */ }
}
function loadUiPreferences() {
  try {
    const saved = JSON.parse(localStorage.getItem('dentiva:ui-prefs') || 'null');
    if (saved && typeof saved === 'object') {
      if (saved.patientColumns && typeof saved.patientColumns === 'object') ui.patientColumns = saved.patientColumns;
      if (saved.patientDensity) ui.patientDensity = saved.patientDensity;
    }
  } catch { /* malformed prefs ignored */ }
}
function patientColumnsEnabled() {
  const saved = { ...patientColumnState(), ...(ui.patientColumns || {}) };
  return new Set(PATIENT_COLUMN_DEFS.filter(([k]) => saved[k] !== false).map(([k]) => k));
}
function patientColumnCell(p, key) {
  switch (key) {
    case 'patient': return `<td><div class="person-cell"><span class="avatar avatar-small">${initials(p.fullName)}</span><div><strong>${esc(p.fullName)}</strong><small>${esc(p.patientCode || '—')}${p.phone ? ` · ${esc(p.phone)}` : ''}</small></div></div></td>`;
    case 'phone': return `<td>${p.phone ? esc(p.phone) : '<span class="muted">—</span>'}${p.email ? `<br><small class="muted">${esc(p.email)}</small>` : ''}</td>`;
    case 'age': return `<td>${p.dateOfBirth ? `${ageFromDate(p.dateOfBirth)}` : '—'}${p.gender ? `<br><small class="muted">${esc(p.gender.slice(0,1))}</small>` : ''}</td>`;
    case 'lastVisit': return `<td>${p.lastVisit ? relativeDate(p.lastVisit) : '<span class="muted">Never</span>'}</td>`;
    case 'nextVisit': return `<td>${p.nextVisit && p.nextVisit >= today() ? date(p.nextVisit) : '<span class="muted">—</span>'}</td>`;
    case 'visits': return `<td class="num">${p.visitsCount !== undefined ? number(p.visitsCount) : '—'}</td>`;
    case 'billed': return `<td class="num">${p.billedCents !== undefined ? currency(centsToMoney(p.billedCents)) : '—'}</td>`;
    case 'paid': return `<td class="num">${p.paidCents !== undefined ? currency(centsToMoney(p.paidCents)) : '—'}</td>`;
    case 'balance': return `<td><strong class="${(p.balanceCents || 0) > 0 ? 'text-warning' : ''}">${currency(centsToMoney(p.balanceCents || 0))}</strong></td>`;
    case 'lastPayment': return `<td>${p.lastPaymentDate ? date(p.lastPaymentDate) : '<span class="muted">—</span>'}</td>`;
    case 'status': return `<td>${statusBadge(p.archived ? 'Archived' : 'Active')}</td>`;
    default: return '';
  }
}
function patientListWantsAggregates(sortKey, cols) {
  return ['visits','visits-desc','billed','billed-desc','paid','paid-desc'].includes(sortKey)
    || ['visits','billed','paid','lastPayment'].some((k) => cols.has(k));
}
...
async function renderPatients() {
  ...
  if (ui.patientTagFilter) filters.tag = ui.patientTagFilter;
  if (ui.patientHasPhone) filters.hasPhone = true;
  if (ui.patientToothFilter) filters.toothStatus = ui.patientToothFilter;
  if (ui.patientFrom) filters.registeredFrom = ui.patientFrom;
  if (ui.patientTo) filters.registeredTo = ui.patientTo;
  if (patientListWantsAggregates(sortKey, cols)) filters.includeAggregates = true;
  const [result, savedViews] = await Promise.all([
    listQuery('patients', { ...filters, sort: sortKey }),
    listQuery('savedFilters', { entity: 'patients' })
  ]);
  const rows = result.rows || [];
  const labels = { patient: 'Patient', phone: 'Phone', age: 'Age/Sex', lastVisit: 'Last visit', nextVisit: 'Next appointment', visits: 'Visits', billed: 'Lifetime bill', paid: 'Paid', balance: 'Due', lastPayment: 'Last payment', status: 'Status' };
  const visibleCols = PATIENT_COLUMN_DEFS.filter(([key]) => cols.has(key));
  const density = ui.patientDensity || 'comfortable';
  const body = rows.map((p) => `<tr class="clickable-row density-${density}" data-action="open-patient-profile" data-id="${attr(p.id)}">
    ${visibleCols.map(([key]) => patientColumnCell(p, key)).join('')}
  ...
  const advanced = ui.patientFiltersOpen ? `<div class="filter-panel">
      <label class="field-label">Tag <input type="text" data-change="patient-tag-filter" value="${attr(ui.patientTagFilter || '')}" placeholder="e.g. VIP"></label>
      <label class="field-label">Registered from <input type="date" data-change="patient-date-from" value="${attr(ui.patientFrom || '')}"></label>
      <label class="field-label">Registered to <input type="date" data-change="patient-date-to" value="${attr(ui.patientTo || '')}"></label>
      <label class="field-label">Tooth status <select data-change="patient-tooth-filter"><option value="">Any</option>${['Planned','In Treatment','Completed','Watch'].map((t) => `<option value="${t}" ${ui.patientToothFilter === t ? 'selected' : ''}>${t}</option>`).join('')}</select></label>
      <label class="field-label checkbox-label"><input type="checkbox" data-change="patient-has-phone" ${ui.patientHasPhone ? 'checked' : ''}> Has phone number</label>
      <button class="chip-button" data-action="patient-filters-clear">${icon('x', 13)}<span>Clear advanced</span></button>
    </div>` : '';
  const columnsPanel = ui.patientColumnsOpen ? `<div class="filter-panel">${PATIENT_COLUMN_DEFS.map(([key, label]) => {
      const locked = key === 'patient' || key === 'status';
      return `<label class="field-label checkbox-label"><input type="checkbox" data-change="patient-column-toggle" data-col="${key}" ${cols.has(key) ? 'checked' : ''} ${locked ? 'disabled title="Always visible"' : ''}> ${esc(label)}</label>`;
    }).join('')}</div>` : '';
  return `<div class="page">
    ...
    ${savedViewsMenu}
    ${toolbar(`
      <label class="search-field"><span>${icon('search', 16)}</span><input type="search" placeholder="Search name, code, phone, email or address" value="${attr(listState.patients.query)}" data-input="list-query" data-collection="patients" aria-label="Search patients"><kbd>⌘ K</kbd></label>
      <select data-change="patient-sort" aria-label="Sort patients">${PATIENT_SORTS.map(([v, l]) => `<option value="${v}" ${sortKey === v ? 'selected' : ''}>${l}</option>`).join('')}</select>
      ...
      <button class="btn btn-ghost btn-small" data-action="patient-filters-toggle" aria-expanded="${ui.patientFiltersOpen ? 'true' : 'false'}">${icon('filter', 14)}<span>Advanced</span></button>
      <button class="btn btn-ghost btn-small" data-action="patient-columns-toggle" aria-expanded="${ui.patientColumnsOpen ? 'true' : 'false'}">${icon('layers', 14)}<span>Columns</span></button>
      <span class="density-toggle" role="group" aria-label="Row density">
        <button class="chip-button ${density === 'comfortable' ? 'selected' : ''}" data-action="patient-density" data-density="comfortable" title="Comfortable">▤</button>
        <button class="chip-button ${density === 'compact' ? 'selected' : ''}" data-action="patient-density" data-density="compact" title="Compact">▥</button>
      </span>`, `<span class="muted">${number(result.total || 0)} patient${(result.total || 0) === 1 ? '' : 's'}</span>`)}
    ${advanced}
    ${columnsPanel}
    ${dataTable([...visibleCols.map(([, label]) => labels[label] || label), ''], body, emptyState('users', 'No matching records', (listState.patients.query || ui.patientTagFilter || ui.patientFrom || ui.patientTo || ui.patientToothFilter || ui.patientHasPhone || ui.patientStatusFilter !== 'all' || ui.patientBalanceFilter !== 'all') ? 'Try adjusting the search or clearing filters.' : 'Add your first patient to begin.', button('Add patient', 'open-patient', 'plus', 'secondary')))}
...
function patientQuickActions(p) {
  ...
  if (can('prescriptions.create')) buttons.push(button('New prescription', 'open-prescription', 'file', 'secondary', `data-id="${attr(p.id)}"`));
  if (can('billing.create')) buttons.push(button('New invoice', 'open-invoice', 'receipt', 'secondary', `data-id="${attr(p.id)}"`));
  if (can('payments.create')) buttons.push(button('Record payment', 'open-payment', 'credit', 'secondary', `data-patient="${attr(p.id)}"`));
  if (can('clinical.edit')) buttons.push(button('Schedule follow-up', 'open-followup', 'flag', 'secondary', `data-id="${attr(p.id)}"`));
  buttons.push(button('Attach file', 'open-attachment-add', 'paperclip', 'secondary', `data-id="${attr(p.id)}"`));
  if (can('billing.view')) buttons.push(button('Statement', 'patient-tab', 'chart', 'ghost', `data-tab="statement"`));
  return buttons.join(' ');
}
...
async function renderPatientTab(p, tab, patient) {
  ...
  switch (tab) {
    case 'overview': {
      ...
      const fin = can('billing.view') ? await q('patientFinancialSummary', { patientId: id }).catch(() => null) : null;
      const finCards = fin && fin.ok !== false ? `<section class="finance-strip" aria-label="Lifetime financial summary">
        ${[
          ['Lifetime billed', fin.billedCents, 'patient-tab', 'billing'],
          ['Lifetime paid', fin.netPaidCents ?? fin.paidCents, 'patient-tab', 'payments'],
          ['Total due', fin.dueCents, 'patient-tab', 'statement'],
...
async function renderPatientVisits(id, patient) {
  ...
  const billingRoll = visits.length && can('billing.view')
    ...
    : {};
  const dentists = [...new Map(visits.map((v) => [v.dentistId || '', staffName(v.dentistId)])).entries()];
  const visitBillingRibbon = (v) => {
  ...
  const rows = visits.map((v, index) => {
    ...
    <div class="visit-card-head" data-action="toggle-visit" data-visit="${index}" role="button" tabindex="0" aria-expanded="${index === 0 ? 'true' : 'false'}">
      <div><strong>${esc(v.visitCode || '—')}${v.reason ? ` · ${esc(v.reason)}` : ''}</strong>
      <small>${dateFull(v.date)} · ${esc(staffName(v.dentistId))}${v.diagnosis ? ` · Dx: ${esc(v.diagnosis)}` : ''}</small></div>
      <span class="visit-toggle-icon">${icon('arrow', 15)}</span>
    </div>
    <div class="visit-card-body ${index === 0 ? 'open' : ''}" id="visit-body-${index}" ${index === 0 ? '' : 'hidden'}>
      ${detailFields.length ? `<dl class="detail-list detail-list-visit">${detailFields.map(([k, v2]) => `<div><dt>${esc(k)}</dt><dd>${esc(v2)}</dd></div>`).join('')}</dl>` : ''}
      ${visitBillingRibbon(v)}
      <div class="row-actions">${button('Print visit', 'print-visit', 'printer', 'link', `data-id="${attr(v.id)}"`)}${button('Prescribe from visit', 'open-prescription', 'file', 'link', `data-id="${attr(id)}" data-visit="${attr(v.id)}"`)}${button('Edit', 'open-visit', 'edit', 'link', `data-id="${attr(v.id)}"`)}</div>
    </div>
  </div>`;
  }).join('');
...
async function renderPatientDental(p, patient) {
  ...
  const FDI = {
    adult: [18, 17, 16, 15, 14, 13, 12, 11, 21, 22, 23, 24, 25, 26, 27, 28, 38, 37, 36, 35, 34, 33, 32, 31, 48, 47, 46, 45, 44, 43, 42, 41],
    primary: [55, 54, 53, 52, 51, 61, 62, 63, 64, 65, 75, 74, 73, 72, 71, 81, 82, 83, 84, 85],
  };
  const chartRows = (set) => set === 'adult' ? [[1, 2, 3, 4, 5, 6, 7, 8, 0, 9, 10, 11, 12, 13, 14, 15, 16], [25, 26, 27, 28, 29, 30, 31, 32, 0, 17, 18, 19, 20, 21, 22, 23, 24]]
    : [[1, 2, 3, 4, 5, 0, 6, 7, 8, 9, 10], [16, 17, 18, 19, 20, 0, 11, 12, 13, 14, 15]];
  const multi = ui.dentalMultiSelect ? (ui.dentalMulti = ui.dentalMulti || []) : (ui.dentalMulti = []);
  const toothBtn = (tooth) => {
    if (!tooth) return `<span aria-hidden="true" style="width:12px;display:inline-block"></span>`;
    const record = statusFor(tooth);
    const fdi = FDI[ui.dentition]?.[tooth - 1];
    const inMulti = multi.includes(tooth);
    return `<button class="tooth ${record ? `tooth-${String(record.status || '').toLowerCase().replace(/\s+/g, '-')}` : ''} ${inMulti ? 'selected' : ''}" data-action="select-tooth" data-tooth="${tooth}" aria-pressed="${inMulti}" title="FDI ${fdi ?? tooth} · Universal ${tooth} — ${record?.status || record?.note || 'No record'}"><span>${fdi ?? tooth}</span></button>`;
  };
  const teeth = Array.from({ length: 32 }, (_, i) => i + 1);
  const chart = (set, label) => `<div class="dental-set"><h3>${esc(label)}${ui.dentition === set ? '' : ` <button class="link-button" data-action="set-dentition" data-dentition="${set}">View</button>`}</h3>${ui.dentition === set ? `<div class="tooth-rows">${chartRows(set).map((row) => `<div class="tooth-grid">${row.map(toothBtn).join('')}</div>`).join('')}</div>` : `<p class="muted">Not currently displayed.</p>`}</div>`;
  const selected = !ui.dentalMultiSelect && ui.dentalTooth ? statusFor(ui.dentalTooth) : null;
  const multiTeeth = multi.slice().sort((a, b) => a - b);
  return `<section class="card dental-card">
    <div class="card-title"><div class="card-title-text">${icon('tooth', 17)}<h2>Dental chart — ${esc(p.fullName)}</h2></div><div class="dentition-toggle"><button class="btn ${ui.dentalMultiSelect ? 'btn-primary' : 'btn-secondary'}" data-action="dental-multi-toggle" aria-pressed="${!!ui.dentalMultiSelect}">${icon('copy', 14)} Multi</button><button class="btn ${ui.dentition === 'adult' ? 'btn-primary' : 'btn-secondary'}" data-action="set-dentition" data-dentition="adult">Adult</button><button class="btn ${ui.dentition === 'primary' ? 'btn-primary' : 'btn-secondary'}" data-action="set-dentition" data-dentition="primary">Primary</button></div></div>
    ${chart('adult', 'Permanent (FDI 11–48)')}${chart('primary', 'Primary (FDI 51–85)')}
    <div class="dental-detail">
      ${ui.dentalMultiSelect ? `<div class="dental-detail-head"><strong>${icon('copy', 15)} Multi-select — ${multiTeeth.length ? `${multiTeeth.length} tooth/teeth (FDI ${multiTeeth.map((t) => FDI[ui.dentition]?.[t - 1] ?? t).join(', ')})` : 'no teeth selected yet'}</strong><span class="muted">Status, procedure and note apply to every selected tooth as separate history entries.</span></div>` : ui.dentalTooth ? `<div class="dental-detail-head"><strong>Tooth FDI ${FDI[ui.dentition]?.[ui.dentalTooth - 1] ?? ui.dentalTooth} · Universal ${ui.dentalTooth} (${ui.dentition})</strong><span class="muted">${records.filter((r) => Number(r.tooth) === Number(ui.dentalTooth) && (r.dentition || 'adult') === ui.dentition).length} record(s) in history</span></div>
      ...
      : emptyState('tooth', 'Select a tooth', 'Click any tooth to record its status, procedure or a chart note — or turn on Multi to update several teeth at once. History is preserved.')}
    </div>
  </section>`;
}
...
async function renderPatientStatement(id, patient) {
  const period = ui.statementPeriod || {};
  const page = ui.statementPage || 1;
  const statement = await q('patientStatement', { patientId: id, page, pageSize: 25, from: period.from || '', to: period.to || '' });
    ...
    <form data-form="settings" class="settings-form">
      <section class="card"><div class="card-title"><div class="card-title-text">${icon('building', 17)}<h2>Clinic identity</h2></div></div>
        <div class="form-grid two">
          ...
          ${field('Dentist registration (BMDC no.)', 'dentistRegistration', s.dentistRegistration, 'text', 'placeholder="e.g. BMDC-12345 — printed on prescriptions"')}
          ${field('Phone', 'phone', s.phone, 'tel')}
          ${field('Website', 'clinicWebsite', s.clinicWebsite, 'url', 'placeholder="clinic.example.com"')}
...
function closeModal() {
  ...
  if (ui.modal?.type === 'print-preview' && ui.rxPreviewReturn) {
    // Returning from an unsaved rx preview: restore the builder with every field intact.
    const form = ui.rxPreviewReturn;
    ui.rxPreviewReturn = null;
    return openModal('prescription', { prescription: form, visitId: form.visitId });
  }
  ui.modal = null;
  render();
}
...
/* --------------------------- prescription --------------------------- */
function rxFrequencyDatalist() {
  const patterns = ['1-0-0', '0-0-1', '1-0-1', '1-1-1', '2-0-2', '1-1-1-1', 'When needed (SOS)', 'Once weekly', 'Morning', 'Night'];
  return `<datalist id="rx-freq-options">${patterns.map((pat) => `<option value="${attr(pat)}">`).join('')}</datalist>`;
}
function rxMedicationRow(item = {}, index = 0) {
  const fq = item.frequencyPattern || item.frequency || '';
  return `<div class="rx-med-row" data-rx-row="${index}">
    <div class="rx-med-grid">
      <label class="field-label">Medicine <input type="text" name="medications[${index}][medicine]" value="${attr(item.medicine || '')}" list="rx-catalog" required aria-label="Medicine name"></label>
      <label class="field-label">Form <select name="medications[${index}][form]">${['', 'Tablet', 'Capsule', 'Syrup', 'Suspension', 'Cream', 'Gel', 'Mouthwash', 'Drops', 'Injection', 'Other'].map((f) => `<option value="${f}" ${String(item.form || '') === f ? 'selected' : ''}>${f || '—'}</option>`).join('')}</select></label>
      <label class="field-label">Strength <input type="text" name="medications[${index}][strength]" value="${attr(item.strength || '')}" placeholder="500 mg" aria-label="Strength"></label>
      <label class="field-label">Dose <input type="text" name="medications[${index}][dosage]" value="${attr(item.dosage || '')}" placeholder="1 tablet" aria-label="Dose"></label>
      <label class="field-label">Frequency <input type="text" name="medications[${index}][frequency]" value="${attr(fq)}" list="rx-freq-options" placeholder="1-1-1" aria-label="Frequency"></label>
      <label class="field-label">Food relation <select name="medications[${index}][foodRelation]">${['', 'Before food', 'After food', 'With food', 'At bedtime', 'Any time'].map((f) => `<option value="${f}" ${String(item.foodRelation || '') === f ? 'selected' : ''}>${f || '—'}</option>`).join('')}</select></label>
      <label class="field-label rx-dur">Duration <span class="rx-inline"><input type="number" name="medications[${index}][durationValue]" value="${attr(String(item.durationValue || ''))}" min="0" max="365" step="1" aria-label="Duration value"><select name="medications[${index}][durationUnit]" aria-label="Duration unit">${[['Days', 'days'], ['Weeks', 'weeks'], ['Months', 'months']].map(([v, l]) => `<option value="${v}" ${String(item.durationUnit || 'Days') === v ? 'selected' : ''}>${l}</option>`).join('')}</select></span></label>
      <label class="field-label rx-qty">Qty <input type="number" name="medications[${index}][quantity]" value="${attr(String(item.quantity || ''))}" min="0" step="1" aria-label="Quantity"></label>
      <label class="field-label form-full">Special instructions <input type="text" name="medications[${index}][instructions]" value="${attr(item.instructions || '')}" placeholder="e.g. Complete the full course" aria-label="Special instructions"></label>
    </div>
    <div class="rx-row-tools">
      <button type="button" class="icon-button" data-action="rx-row-move" data-dir="-1" aria-label="Move up">${icon('arrow', 14, 'rotate-180')}</button>
      <button type="button" class="icon-button" data-action="rx-row-move" data-dir="1" aria-label="Move down">${icon('arrow', 14)}</button>
      <button type="button" class="icon-button" data-action="rx-row-duplicate" aria-label="Duplicate medicine">${icon('copy', 14)}</button>
      <button type="button" class="icon-button danger" data-action="rx-row-remove" aria-label="Remove medicine">${icon('trash', 14)}</button>
    </div>
  </div>`;
}
function rxRowIndexes(container) { return [...container.querySelectorAll('.rx-med-row')]; }
function renumberRxRows(container) {
  rxRowIndexes(container).forEach((row, index) => {
    row.dataset.rxRow = String(index);
    row.querySelectorAll('[name]').forEach((el) => { el.name = el.name.replace(/medications\[\d+\]/, `medications[${index}]`); });
  });
}
function rxPickerSync(pickerEl) {
  if (!pickerEl) return;
  const hidden = pickerEl.querySelector('input[type="hidden"]');
  const custom = pickerEl.querySelector('.rx-picker-custom');
  const chosen = [...pickerEl.querySelectorAll('.chip-button.selected')].map((chip) => chip.dataset.opt);
  if (hidden) hidden.value = [...chosen, ...(custom && custom.value.trim() ? [custom.value.trim()] : [])].join('; ');
}
function rxRowFromContainer() {
  const container = document.querySelector('#rx-med-rows');
  if (!container) return [];
  return rxRowIndexes(container).map((row) => ({
    medicine: row.querySelector('[name$="[medicine]"]')?.value.trim() || '',
    form: row.querySelector('[name$="[form]"]')?.value || '',
    strength: row.querySelector('[name$="[strength]"]')?.value.trim() || '',
    dosage: row.querySelector('[name$="[dosage]"]')?.value.trim() || '',
    frequency: row.querySelector('[name$="[frequency]"]')?.value.trim() || '',
    foodRelation: row.querySelector('[name$="[foodRelation]"]')?.value || '',
    durationValue: row.querySelector('[name$="[durationValue]"]')?.value || '',
    durationUnit: row.querySelector('[name$="[durationUnit]"]')?.value || 'Days',
    quantity: row.querySelector('[name$="[quantity]"]')?.value || '',
    instructions: row.querySelector('[name$="[instructions]"]')?.value.trim() || ''
  })).filter((r) => r.medicine);
}
// Prescription clinical presets — terminology is specified by the practice; do not rename.
const RX_CC_OPTIONS = ['Pain On', 'G. Carries', 'Swelling', 'Gum Bleeding', 'Bad Breath', 'Sensitivity'];
const RX_OE_OPTIONS = ['Carries / G Carries', 'BDR / BDC', 'Gingivitis', 'Parodental Pocket', 'Perio Dontitis', 'Pulpitis', 'Impected Teeth', 'Dry Socket', 'Attrition / Erosion'];
/** Chip-based clinical multi-select: presets toggle in, custom text appends; hidden field holds the joined value. */
function rxClinicalPicker(name, label, presets, stored, placeholder) {
  const tokens = String(stored || '').split(/;\s*/).map((t) => t.trim()).filter(Boolean);
  const chosen = tokens.filter((t) => presets.includes(t));
  const custom = tokens.filter((t) => !presets.includes(t)).join('; ');
  return `<div class="rx-picker" data-picker="${attr(name)}">
    <label class="field-label">${esc(label)}</label>
    <div class="rx-picker-chips" role="group" aria-label="${attr(label)}">${presets.map((opt) => `<button type="button" class="chip-button ${chosen.includes(opt) ? 'selected' : ''}" data-action="rx-opt-toggle" data-opt="${attr(opt)}" aria-pressed="${chosen.includes(opt)}">${esc(opt)}</button>`).join('')}</div>
    <input type="hidden" name="${attr(name)}" value="${attr(stored || '')}">
    <input type="text" class="rx-picker-custom" data-input="rx-custom-${attr(name)}" value="${attr(custom)}" placeholder="${attr(placeholder)}" aria-label="${attr(label)} custom">
  </div>`;
}
function modalPrescription(data = {}) {
  ...
  const patientId = rx.patientId || data.patientId || ui.patientId;
  const patientVisits = patientId ? (appState.patientVisitsCache?.[patientId] || []) : [];
...
/* ------------------------------ payment ------------------------------ */
const METHOD_REF_HINTS = {
  Cash: 'Optional note (e.g. receipt book page).', Bank: 'Cheque / deposit slip reference.', Card: 'Last 4 digits or auth code.',
  bKash: 'bKash transaction ID (e.g. 9AB12XYZ34).', Nagad: 'Nagad transaction ID.', Rocket: 'Rocket transaction ID.', Upay: 'Upay transaction ID.', Other: 'How will the clinic reconcile this payment?'
};
function modalPayment(data = {}) {
  ...
  return `<form data-form="payment"><input type="hidden" name="invoiceId" value="${attr(p.invoiceId || data.invoiceId || '')}">${modalHead('PAYMENT', 'Record payment', 'Overpayment is blocked; the receipt number stays sequential.')}
    <div class="form-grid two">
      ...
      <label class="field-label">Reference <input type="text" name="reference" value="${attr(p.reference || '')}" placeholder="${attr(METHOD_REF_HINTS[method] || METHOD_REF_HINTS.Other)}"><span class="method-ref-hint" id="payment-ref-hint">${esc(METHOD_REF_HINTS[method] || '')}</span></label>
      ${field('Amount', 'amount', p.amount || (due ? centsToMoney(due) : ''), 'number', 'min="0.01" step="0.01" required')}
      ${field('Notes', 'notes', p.notes, 'textarea', 'rows="2"')}
    </div>
    ${inv ? `<div class="form-note">Invoice ${esc(inv.invoiceNumber || '')} — total ${currency(centsToMoney(inv.totalCents ?? inv.total))}, paid ${currency(centsToMoney(inv.paidCents ?? inv.paid ?? 0))}, due ${currency(centsToMoney(due ?? inv.due))}. Amount prefilled to the remaining due; edit for a partial payment.</div>` : ''}
    <div class="form-note">${icon('shield', 14)} Recording is guarded against double submission and audited with your name.</div>
...
const COMMAND_ACTIONS = [
  { keys: ['new patient', 'register patient', 'add patient', 'patient nibc'], label: 'New patient', sub: 'Register a patient record', action: 'open-patient', icon: 'users', permission: 'patients.create' },
  { keys: ['new appointment', 'book appointment', 'schedule'], label: 'New appointment', sub: 'Book a slot on the schedule', action: 'open-appointment', icon: 'calendar', permission: 'appointments.create' },
  { keys: ['new visit', 'record visit', 'clinical record'], label: 'Record visit', sub: 'Create a clinical visit', action: 'open-visit', icon: 'clipboard', permission: 'patients.view' },
  { keys: ['new prescription', 'prescribe', 'rx'], label: 'New prescription', sub: 'Structured medication builder', action: 'open-prescription', icon: 'file', permission: 'prescriptions.create' },
  { keys: ['new invoice', 'create invoice', 'bill'], label: 'New invoice', sub: 'Create a bill for a patient', action: 'open-invoice', icon: 'receipt', permission: 'billing.create' },
  { keys: ['record payment', 'payment', 'receipt'], label: 'Record payment', sub: 'Collect a payment', action: 'open-payment', icon: 'credit', permission: 'payments.create' },
  { keys: ['queue', 'today queue', 'chamber queue'], label: 'Today’s queue', sub: 'Queue and chair flow', action: 'navigate', icon: 'clock', page: 'queue' },
  { keys: ['dashboard', 'home', 'command centre'], label: 'Dashboard', sub: 'Workspace command centre', action: 'navigate', icon: 'grid', page: 'dashboard' },
  { keys: ['reports', 'analytics'], label: 'Reports', sub: 'Revenue and operations reports', action: 'navigate', icon: 'chart', page: 'reports', permission: 'reports.view' },
  { keys: ['backup', 'backup now'], label: 'Backup now', sub: 'Create an offline backup', action: 'navigate', icon: 'backup', page: 'backup' },
  { keys: ['settings', 'clinic setup'], label: 'Settings', sub: 'Clinic, documents and preferences', action: 'navigate', icon: 'settings', page: 'settings', permission: 'settings.view' },
  { keys: ['audit', 'activity log'], label: 'Audit trail', sub: 'Who changed what, when', action: 'navigate', icon: 'shield', page: 'audit', permission: 'audit.view' },
];
function commandActionsFor(qText) {
  const needle = String(qText || '').trim().toLowerCase();
  return COMMAND_ACTIONS.filter((cmd) => !cmd.permission || can(cmd.permission))
    .filter((cmd) => !needle || cmd.keys.some((key) => key.includes(needle)) || cmd.label.toLowerCase().includes(needle))
    .slice(0, needle ? 5 : 6);
}
async function globalSearch(qText) {
  const needle = String(qText || '').trim();
  const actions = commandActionsFor(needle);
  if (needle.length < 2) return { actions };
  const found = await q('globalSearch', { query: needle, limit: 5 }).catch(() => null);
...
async function handleSubmit(event) {
  ...
  // Duplicate-submission guard: a slow double-click on Save must not create
  // two payments/invoices/prescriptions. Guard is renderer-wide and resets
  // even when the op throws (finally below).
  if (ui.submitting) return;
  ui.submitting = true;
  event.target.querySelectorAll('button[type="submit"]').forEach((btn) => { btn.disabled = true; });
  try { await handleSubmitInner(event); }
  finally { ui.submitting = false; }
}

async function handleSubmitInner(event) {
  ...
  if (formName === 'prescription') {
    ...
    for (let index = 0; ; index += 1) {
      const medicine = data[`medications[${index}][medicine]`];
      if (medicine === undefined) break;
      medications.push({
        medicine: String(medicine || '').trim(),
        form: data[`medications[${index}][form]`] || '',
        strength: data[`medications[${index}][strength]`] || '',
        dosage: data[`medications[${index}][dosage]`] || '',
        frequency: data[`medications[${index}][frequency]`] || '',
        foodRelation: data[`medications[${index}][foodRelation]`] || '',
        durationValue: data[`medications[${index}][durationValue]`] || '',
        durationUnit: data[`medications[${index}][durationUnit]`] || '',
        quantity: data[`medications[${index}][quantity]`] || '',
        instructions: data[`medications[${index}][instructions]`] || ''
      });
      if (index > 64) break; /* DOM-bound guard; ops clamps to 40 */
    }
    const payload = { ...data, medications: medications.filter((m) => m.medicine) };
...
async function handleClick(event) {
  ...
  switch (action) {
    ...
    case 'open-prescription': {
      const visitId = target.dataset.visit || '';
      const patientId = String(id || '').startsWith('patient_') ? id : ui.patientId;
      const prescription = id && !String(id).startsWith('patient_') ? (await q('record', { collection: 'prescriptions', id })).record : {};
      const patientKey = patientId || prescription.patientId || '';
      if (patientKey) {
        const visits = await q('list', { collection: 'visits', page: 1, pageSize: 30, filters: { patientId: patientKey }, sort: 'date-desc' }).catch(() => ({ rows: [] }));
        appState.patientVisitsCache = { ...(appState.patientVisitsCache || {}), [patientKey]: visits.rows || [] };
      }
      return openModal('prescription', { prescription, visitId, patientId });
    }
    case 'toggle-visit': {
      const head = target.closest('.visit-card-head');
      const body = head?.nextElementSibling;
      if (body) { const open = body.classList.toggle('open'); body.hidden = !open; head.setAttribute('aria-expanded', String(open)); }
      return;
    }
    case 'statement-period-clear': ui.statementPeriod = {}; ui.statementPage = 1; return render();
    case 'statement-page': ui.statementPage = Math.max(1, Number(target.dataset.page) || 1); return render();
    case 'export-patient-statement-pdf': { const patient = ui.patientDetail?.patient; if (patient) return printPatientStatement(patient, { from: ui.statementPeriod?.from || '', to: ui.statementPeriod?.to || '' }); return; }
    case 'visit-filter-clear': ui.visitFilters = {}; return render();
    ...
    case 'select-tooth': {
      ...
      if (ui.dentalMultiSelect) {
        const set = new Set(ui.dentalMulti || []);
        if (set.has(tooth)) set.delete(tooth); else set.add(tooth);
        ui.dentalMulti = [...set];
        return render();
      }
      ui.dentalTooth = tooth;
      return render();
    }
    case 'set-dentition': ui.dentition = target.dataset.dentition; ui.dentalTooth = null; ui.dentalMulti = []; return render();
    case 'dental-multi-toggle': ui.dentalMultiSelect = !ui.dentalMultiSelect; ui.dentalMulti = []; if (ui.dentalMultiSelect) ui.dentalTooth = null; return render();
    ...
    case 'save-tooth': {
      ...
      if (!patientId) return notify('Choose a patient first.', 'error');
      if (ui.dentalMultiSelect) {
        const teeth = [...(ui.dentalMulti || [])].sort((a, b) => a - b);
        if (!teeth.length) return notify('Select at least one tooth on the chart.', 'error');
        let saved = 0;
        for (const tooth of teeth) {
          const result = await op('dental.save', { patientId, tooth, dentition: ui.dentition, status: ui.toothStatus || '', note, procedure });
          if (!result) break;
          saved += 1;
        }
        if (saved === teeth.length) { ui.toothStatus = undefined; ui.toothNote = undefined; ui.dentalMulti = []; notify(`Chart updated for ${saved} teeth — each entry recorded separately with full history.`); return render(); }
        return;
      }
      if (!ui.dentalTooth) return notify('Choose a tooth first.', 'error');
      ...
      if (result) { ui.toothStatus = undefined; ui.toothNote = undefined; notify(`Tooth record saved.`); return render(); }
      return;
    }
    case 'remove-tooth': {
    ...
    case 'rx-opt-toggle': {
      ...
      rxPickerSync(target.closest('.rx-picker'));
      return;
    }
    case 'rx-row-add': {
      const container = document.querySelector('#rx-med-rows');
      if (!container) return;
      container.insertAdjacentHTML('beforeend', rxMedicationRow({}, rxRowIndexes(container).length));
      renumberRxRows(container);
      return;
    }
    case 'rx-row-remove': {
      const row = target.closest('.rx-med-row');
      const container = document.querySelector('#rx-med-rows');
      if (row && container) { row.remove(); if (!rxRowIndexes(container).length) container.insertAdjacentHTML('beforeend', rxMedicationRow({}, 0)); renumberRxRows(container); }
      return;
    }
    case 'rx-row-duplicate': {
      const row = target.closest('.rx-med-row');
      const container = document.querySelector('#rx-med-rows');
      if (row && container) {
        const data = {};
        row.querySelectorAll('[name]').forEach((el) => { data[el.name.replace(/^medications\[\d+\]\[(\w+)\]$/, '$1')] = el.value; });
        container.insertAdjacentHTML('beforeend', rxMedicationRow(data, rxRowIndexes(container).length));
        renumberRxRows(container);
      }
      return;
    }
    case 'rx-row-move': {
      ...
      const dir = Number(target.closest('[data-action]')?.dataset.dir || target.dataset.dir || 0);
      const neighbour = dir < 0 ? row.previousElementSibling : row.nextElementSibling;
      if (neighbour) {
        if (dir < 0) container.insertBefore(row, neighbour);
        else container.insertBefore(neighbour, row.nextSibling === neighbour ? row : neighbour.nextSibling);
        renumberRxRows(container);
      }
      return;
    }
    case 'rx-template-apply': {
      const select = document.querySelector('#rx-template-select');
      const template = (appState.settings.medicationTemplates || []).find((t) => t.id === select?.value);
      if (!template) return notify('Choose a template first.', 'info');
      const container = document.querySelector('#rx-med-rows');
      if (container) { container.innerHTML = template.medications.map((item, i) => rxMedicationRow(item, i)).join(''); renumberRxRows(container); }
      if (template.advice) { const el = document.querySelector('form[data-form="prescription"] [name="advice"]'); if (el && !el.value) el.value = template.advice; }
      if (template.followUp) { const el = document.querySelector('form[data-form="prescription"] [name="followUp"]'); if (el && !el.value) el.value = template.followUp; }
      return notify(`Template "${template.name}" applied — review everything before saving.`, 'info');
    }
    case 'rx-template-save': {
      ...
      (async () => {
        ...
        if (!result) return;
        appState.settings = { ...appState.settings, medicationTemplates: result.record ? [...(appState.settings.medicationTemplates || []), result.record] : appState.settings.medicationTemplates };
        notify(`Template "${name.trim()}" saved.`);
        render();
      })();
      return;
    }
    case 'rx-preview': {
      ...
      openDocumentPreview('Prescription preview', previewSpec);
      ui.modalStackAfterPrint = currentModal?.type === 'prescription' ? currentModal : null;
      notify('Preview reflects the unsaved form — nothing is stored until you save.', 'info');
      return;
    }
...
async function handleInput(event) {
  ...
  if (key && key.startsWith('rx-custom-')) {
    rxPickerSync(target.closest('.rx-picker'));
    return;
  }
...
async function handleChange(event) {
  ...
  recordActivity();
  switch (key) {
    case 'payment-method': { const hint = document.getElementById('payment-ref-hint'); if (hint) hint.textContent = METHOD_REF_HINTS[target.value] || METHOD_REF_HINTS.Other; return; }
    case 'patient-sort': ui.patientSort = target.value; listState.patients.page = 1; return render();
    case 'patient-tag-filter': ui.patientTagFilter = target.value.trim(); listState.patients.page = 1; return render();
    case 'patient-date-from': ui.patientFrom = target.value; listState.patients.page = 1; return render();
    case 'patient-date-to': ui.patientTo = target.value; listState.patients.page = 1; return render();
...
function handleKeydown(event) {
  ...
  if (ui.modal?.type === 'search' && ['ArrowDown', 'ArrowUp', 'Enter'].includes(event.key)) {
    const rows = [...document.querySelectorAll('#command-results .command-row')];
...
function commandResultsHtml(results) {
  const sections = [];
  if (results.actions?.length) sections.push(`<div class="command-section"><h4>Actions</h4>${results.actions.map((cmd) => `<button class="command-row" data-action="${cmd.action === 'navigate' ? 'navigate' : cmd.action}" ${cmd.page ? `data-page="${attr(cmd.page)}"` : ''}>${icon(cmd.icon, 15)}<span><strong>${esc(cmd.label)}</strong><small>${esc(cmd.sub)}</small></span><span class="command-kind">Run</span></button>`).join('')}</div>`);
  const sectionOf = (label, rows, action, extra = () => '') => rows?.length ? `<div class="command-section"><h4>${esc(label)}</h4>${rows.map((item) => `<button class="command-row" data-action="${action}" data-id="${attr(item.id)}" ${extra(item)}><strong>${esc(item.label)}</strong><small>${esc(item.sub || '')}</small></button>`).join('')}</div>` : '';
  sections.push(sectionOf('Patients', results.patients, 'open-patient-profile'));
  sections.push(sectionOf('Appointments', results.appointments, 'open-appointment-detail'));
  sections.push(sectionOf('Invoices', results.invoices, 'open-invoice-detail'));
  sections.push(sectionOf('Payments', results.payments, 'print-payment'));
  sections.push(sectionOf('Prescriptions', results.prescriptions, 'open-prescription'));
  sections.push(sectionOf('Visits', results.visits, 'open-patient-profile'));
  const out = sections.filter(Boolean).join('');
  return out || `<p class="muted">No matching records — try a patient name, phone number, receipt or action.</p>`;
}
...
function openDocumentPreview(title, spec) {
  ...
  openModal('print-preview', { title, pageSize: spec.pageSize || defaultPrintPageSize(), docSpec: spec });
}
function renderDocumentSpec(data) {
  if (!data?.docSpec) return null;
  return buildDocument({ settings: appState.settings, lang: docLang(), footerStamp: dateFull(today()), ...data.docSpec, pageSize: data.pageSize || data.docSpec.pageSize || defaultPrintPageSize() }).html;
}
...
async function executePrint(mode) {
  ...
  const markup = renderDocumentSpec(data) || buildPrintDocument(data.title, data.content, data.pageSize);
  let result;
  try {
...
async function printInvoice(inv) {
  const patient = ui.patientDetail?.patient?.id === inv.patientId ? ui.patientDetail.patient
    : ((await q('record', { collection: 'patients', id: inv.patientId })).record || {});
  const pairs = [];
  if (inv.subtotal !== undefined) pairs.push(['Subtotal', currency(inv.subtotal)]);
  if ((inv.discountCents ?? 0) > 0 || (inv.discount || 0) > 0) pairs.push(['Discount', currency(centsToMoney(inv.discountCents ?? moneyToCents(inv.discount || 0)))]);
  if ((inv.taxCents ?? 0) > 0 || (inv.tax || 0) > 0) pairs.push([`Tax${inv.taxRate ? ` (${inv.taxRate}%)` : ''}`, currency(centsToMoney(inv.taxCents ?? moneyToCents(inv.tax || 0)))]);
  const receipts = await q('list', { collection: 'payments', page: 1, pageSize: 100, filters: { invoiceId: inv.id } }).catch(() => ({ rows: [] }));
  const receiptRows = (receipts.rows || []).map((pay) => ({ receiptNumber: pay.receiptNumber, date: date(pay.date), method: pay.method, amount: currency(centsToMoney(Math.max(0, (pay.amountCents ?? 0) - (pay.refundedCents ?? 0)))) }));
  const body = invoiceLines({
    items: (inv.items || []).map((item) => ({ name: item.name, category: item.category, quantity: item.quantity, unitPrice: item.unitPrice, unitPriceCents: item.unitPriceCents, lineTotalCents: item.lineTotalCents !== undefined ? item.lineTotalCents : undefined })),
...
async function printPayment(payment) {
  const refunded = payment.refundedCents !== undefined ? centsToMoney(payment.refundedCents) : (Number(payment.refundedAmount) || 0);
  const patient = ui.patientDetail?.patient?.id === payment.patientId ? ui.patientDetail.patient
    : ((await q('record', { collection: 'patients', id: payment.patientId }).catch(() => ({}))).record || {});
...
async function printPatientStatement(patient, { from = '', to = '' } = {}) {
  const rows = [];
  let page = 1;
  let total = 0;
  let agg = {};
  for (;;) {
    const result = await q('patientStatement', { patientId: patient.id, page, pageSize: 500, from, to });
