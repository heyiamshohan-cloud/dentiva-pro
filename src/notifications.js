import { clinicDate, DEFAULT_TIMEZONE } from './core.js';
...
/** Pure calendar arithmetic on a clinic-day string (no timezone involved). */
const addDays = (day, days) => {
  const [year, month, date] = String(day).split('-').map(Number);
  const shifted = new Date(Date.UTC(year, (month || 1) - 1, date || 1) + days * 86400000);
...
function notify(kind, type, priority, title, message, page, day, now, extra = {}) {
  return {
    id: `auto_${kind}_${day}`,
 ...
 */
export function deriveNotifications(repo, settings = {}, { now = new Date(), today, threshold = Number(settings.lowStockThreshold) || 5 } = {}) {
  const rules = normalizeNotificationRules(settings.notificationRules);
  // `day` is the CLINIC's calendar day (settings timezone), never the machine's
  // local day -- a workspace in Dhaka running on a UTC host must signal the day
  // the clinic is actually working. Callers inside the operation registry pass
  // ctx.today(); the fallback derives the same value from the settings timezone.
  const day = today || clinicDate(now, settings.timezone || DEFAULT_TIMEZONE);
  const rows = [];
  const errors = [];
  ...
  guarded('appointments', () => {
    const scheduled = total(repo.listCollection('appointments', { page: 1, pageSize: 1, filters: { date: day, status: 'Scheduled' } }));
    const confirmed = total(repo.listCollection('appointments', { page: 1, pageSize: 1, filters: { date: day, status: 'Confirmed' } }));
    const count = scheduled + confirmed;
    if (!count) return null;
    return notify('appointments', 'bell', 'normal', `${count} appointment${count === 1 ? '' : 's'} scheduled today`,
      `${scheduled} scheduled and ${confirmed} confirmed for ${day}. Keep the chair moving — check patients in from Today’s Queue.`,
      'queue', day, now);
  });

  guarded('followups', () => {
    const due = total(repo.listCollection('followUpTasks', { page: 1, pageSize: 1, filters: { dueBefore: day, status: 'Open' } }));
    if (!due) return null;
    return notify('followups', 'warning', 'high', `${due} clinical follow-up${due === 1 ? '' : 's'} due`,
      `Follow-up tasks with a due date on or before ${day} need a call or a booked visit.`,
      'patients', day, now);
  });
  ...
  guarded('expiry', () => {
    const windowEnd = addDays(day, 30);
  ...
  guarded('queue', () => {
    const checkedIn = total(repo.listCollection('appointments', { page: 1, pageSize: 1, filters: { date: day, status: 'Checked In' } }));
    const waiting = total(repo.listCollection('appointments', { page: 1, pageSize: 1, filters: { date: day, status: 'Waiting' } }));
