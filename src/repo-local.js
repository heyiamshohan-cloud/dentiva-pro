...
export class LocalRepo {
  ...
  listPatients({ query = '', status = 'all', balance = 'all', dateFrom = '', dateTo = '', registeredFrom = '', registeredTo = '', hasPhone = false, toothStatus = '', tag = '', page = 1, pageSize = 50, sort = 'name', includeAggregates = false } = {}) {
    ...
    const withAggregates = includeAggregates || ['visits', 'visits-desc', 'billed', 'billed-desc', 'paid', 'paid-desc'].includes(sort);
    let hydrated = records.map((record) => hydrate(record, 'patients'));
    if (withAggregates) {
      const visits = this.#collection('visits');
      const invoices = this.#collection('invoices');
      const payments = this.#collection('payments');
      hydrated = hydrated.map((record) => {
        if (!record) return record;
        const myPayments = payments.filter((pay) => pay && pay.patientId === record.id && pay.status !== 'Voided' && pay.status !== 'Cancelled');
        return {
          ...record,
          visitsCount: visits.filter((visit) => visit && visit.patientId === record.id).length,
          billedCents: toCents(invoices.filter((inv) => inv && inv.patientId === record.id && inv.status !== 'Cancelled').reduce((sum, inv) => sum + (inv.totalCents ?? (inv.total || 0) * 100), 0)),
          paidCents: toCents(myPayments.reduce((sum, pay) => sum + (pay.amountCents ?? (pay.amount || 0) * 100), 0)),
          lastPaymentDate: myPayments.reduce((last, pay) => (pay.date > last ? pay.date : last), '')
        };
      });
    }
