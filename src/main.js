 ...
 */
import './styles.css';
import { ARRAY_COLLECTIONS, CURRENT_SCHEMA_VERSION, applyRestorePlan, buildBackupManifest, buildRestorePlan, appointmentsOverlap, calculateInvoice, canAcceptPayment, canonicalJson, paymentStatusFor, sanitizeFilename, validateAttachmentFile, validateBackupPayload, validateMoney, validatePayment, validateRelationships } from './core.js';
...
function render() { app.innerHTML = state.unsupportedSchema ? unsupportedSchemaScreen() : (ui.locked ? lockScreen() : shell()); translateDom(); document.title = `${ui.locked ? 'Workspace locked' : pageTitle()} · Dentiva Pro`; if (!ui.locked) resetActivityTimer(); }
