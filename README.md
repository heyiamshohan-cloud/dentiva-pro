...
**Premium offline-first dental practice management for Windows · v1.5.2**

Dentiva Pro is a local, relational dental-practice workspace for clinics in Bangladesh. v1.5.2 carries the **forensic hardening** from the final audit (working date formats, no dead code, predictable print errors, strict dependency gate) on top of v1.5.0, which builds on the v1.4.0 **relational SQLite engine** (Node's built-in `node:sqlite`, zero native dependencies), a **shared service layer** with server-side validation, RBAC and audit, and a rebuilt **async, paginated renderer** on a light-only Design System 2.0.
