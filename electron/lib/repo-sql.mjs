  ...
  listPatients({ query = '', status = 'all', balance = 'all', dateFrom = '', dateTo = '', registeredFrom = '', registeredTo = '', hasPhone = false, toothStatus = '', tag = '', page = 1, pageSize = 50, sort = 'name', includeAggregates = false, count = true } = {}) {
    ...
    if (needsAggregates) {
      rows = this.ws.query(`SELECT patients.*,
          COALESCE(vc.n, 0) AS visits_count,
          COALESCE(ib.billed, 0) AS billed_cents,
          COALESCE(pb.paid, 0) AS paid_cents,
          pb.last_payment AS last_payment_date
        FROM patients
        LEFT JOIN (SELECT patient_id, COUNT(*) AS n FROM visits GROUP BY patient_id) vc ON vc.patient_id = patients.id
        LEFT JOIN (SELECT patient_id, SUM(CASE WHEN status <> 'Cancelled' THEN total_cents ELSE 0 END) AS billed FROM invoices GROUP BY patient_id) ib ON ib.patient_id = patients.id
        LEFT JOIN (SELECT patient_id, SUM(amount_cents) AS paid, MAX(date) AS last_payment FROM payments WHERE status NOT IN ('Voided','Cancelled') GROUP BY patient_id) pb ON pb.patient_id = patients.id
        ${where.join(' AND ') ? `WHERE ${where.join(' AND ')}` : ''}
        ORDER BY ${orderSql.replace(/visits_count|billed_cents|paid_cents/g, (m) => m)}
        LIMIT ? OFFSET ?`, [...params, pageSize, (Math.max(1, page) - 1) * pageSize])
        .map((row) => { const { visits_count, billed_cents, paid_cents, last_payment_date, ...rest } = row; const rec = rowToRecord(rest); return rec ? { ...rec, visitsCount: Number(visits_count) || 0, billedCents: Number(billed_cents) || 0, paidCents: Number(paid_cents) || 0, lastPaymentDate: last_payment_date || '' } : null; })
        .filter(Boolean);
    } else {
